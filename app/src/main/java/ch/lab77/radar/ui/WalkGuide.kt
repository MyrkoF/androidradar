 ...
 */
@Composable
fun WalkGuide(d: Device, st: ScanStatus) {
    ...
    Column(Modifier.fillMaxWidth().padding(top = 6.dp), verticalArrangement = Arrangement.spacedBy(4.dp)) {
        Text("Guide de marche", color = Palette.green, fontFamily = FontFamily.Monospace, fontSize = 12.sp)
        Text(trendText, color = trendColor, fontFamily = FontFamily.Monospace, fontSize = 12.sp)
        ...
        if (st.heading == null) {
            ...
            Column {
                if (filled < 4) Text("Tournez lentement sur vous-même,\ntéléphone devant vous (${filled}/12 secteurs).", color = Palette.text, fontFamily = FontFamily.Monospace, fontSize = 11.sp)
                else Text("Signal le plus fort vers ${best?.let { it * 30 } ?: "?"}° (±30°).\nAvancez dans cette direction, puis refaites un tour.", color = Palette.text, fontFamily = FontFamily.Monospace, fontSize = 11.sp)
                Text("Nord en haut · barre verte = secteur le plus fort · aiguille = mon cap", color = Palette.muted, fontFamily = FontFamily.Monospace, fontSize = 10.sp)
            }
        }
    }
}
